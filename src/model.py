...
CLASS_LABELS = ['class_0', 'class_1', 'class_2', 'class_3', 'class_4',
                'class_5', 'class_6', 'class_7', 'class_8', 'class_9']  # Example for CIFAR-10

def load_model(model_path: str = None):
    global MODEL
    if MODEL is None:
        # Default model path, can be overridden by environment variable
        effective_model_path = model_path if model_path else os.environ.get(
...
def predict_image(preprocessed_image: np.ndarray):
    ...
    probabilities = predictions[0].tolist()  # list for JSON
    return {
